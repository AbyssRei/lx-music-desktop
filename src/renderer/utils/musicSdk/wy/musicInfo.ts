...
export default (songmid: string | number): any => {
  const requestObj: any = httpFetch('https://music.163.com/weapi/v3/song/detail', {
    method: 'post',
    headers: {
      'User-Agent':
  ...
  requestObj.promise = requestObj.promise.then(({ body }: { body: any }) => {
    // console.log(body)
