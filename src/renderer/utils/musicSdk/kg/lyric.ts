...
export default {
  getIntv(interval: string): number {
    ...
    while (intvArr.length) {
      intv += (intvArr.pop() as any) * unit
      unit *= 60
    }
    return parseInt(intv as any)
  },
  ...
  //   return requestObj
  // },
  searchLyric(name: string, hash: string, time: any, tryNum: number = 0): any {
  ...
  getLyricDownload(id: any, accessKey: string, fmt: string, tryNum: number = 0): any {
    let requestObj = httpFetch(
  ...
  getLyric(songInfo: any, tryNum: number = 0): any {
