...
export default {
  ...
  async getComment({ hash }: { hash: string }, page: number = 1, limit: number = 20): Promise<any> {
  ...
  async getHotComment({ hash }: { hash: string }, page: number = 1, limit: number = 20): Promise<any> {
    // console.log(songmid)
  ...
  replaceAt(raw: string, atList: any[]): string {
    atList.forEach((atobj: any) => {
