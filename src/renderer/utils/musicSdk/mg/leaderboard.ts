...
export default {
  ...
  getUrl(id: string, page: number): string {
  ...
  getDetailPageUrl(id: string | number): string | null {
    if (typeof id == 'string') id = id.replace('mg__', '')
    for (const item of boardList) {
      if (item.bangid == id) {
        return `https://music.migu.cn/v3/music/top/${item.bangid}`
      }
    }
    return null
  },
