...
const cachedDetailInfo: Record<string, any> = {}
...
export default {
  ...
  getSongListUrl(sortId: string, tagId: string, page: number): string {
    // if (tagId == null) {
  ...
  getSongListDetailUrl(id: string, page: number): string {
  ...
  async getListDetailList(id: string, page: number, tryNum: number = 0): Promise<any> {
    ...
    return requestObj_listDetail.promise.then(async({ body }: any) => {
  ...
  async getListDetailInfo(id: string, tryNum: number = 0): Promise<any> {
    if (tryNum > 2) return Promise.reject(new Error('try max num'))
    ...
    return requestObj_listDetailInfo.promise.then(async({ body }: any) => {
      if (body.code !== this.successCode) return this.getListDetailInfo(id, ++tryNum)
      // console.log(JSON.stringify(body))
      // console.log(body)
  ...
  async getDetailUrl(link: string, page: number, retryNum: number = 0): Promise<any> {
  ...
  filterList2(listData: any[], list: any[] = [], ids = new Set<string>()): any[] {
  ...
  filterTagInfo(rawList: any[]) {
    return {
      hotTag: rawList[0].content.map(({ texts: [name, id] }: any) => ({
        id,
        name,
        source: 'mg',
      })),
      tags: rawList.slice(1).map(({ header, content }: any) => ({
        name: header.title,
        list: content.map(({ texts: [name, id] }: any) => ({
  ...
  async search(text: any, page: any, limit = 20) {
