...
const sortQualityArray = (array: any[]): any[] => {
  ...
  const rawQualityArray: any[] = []
  const newQualityArray: any[] = []
  ...
  rawQualityArray.forEach((item: any) => {
...
export default {
  ...
  filterData(rawList: any[]): any[] {
    return rawList.map((item: any) => {
      ...
      item.n_minfo.split(';').forEach((i: string) => {
  ...
  filterBoardsData(rawList: any[]): any[] {
    // console.log(rawList)
    let list: any[] = []
