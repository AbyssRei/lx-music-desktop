...
export default {
  ...
  getListUrl(id: string | number, page: number): string {
  ...
  async getRecommendList(page: number): Promise<any> {
    ...
    return {
      list:
      body.playlist.data.FeedRsp.List?.map(({ Playlist }: any) => ({
  ...
  //   }
  // },
  filterList({ content }: any, page: number): any {
